...
# Usage: run_smoke_rq1.sh <DATASET: MNIST|CIFAR10|CIFAR100> <TAG> <GPU_ID> [NUM_ROUNDS]
...
# Defaults for smoke obstacle-clearing runs
case "$DATASET" in
  MNIST)
    : "${NUM_ROUNDS:=5}"
    MODEL="SimpleCNN"
...
# Optional PoL smoke overrides via env
