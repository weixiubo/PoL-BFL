...
# Reduced-scale verification of attack application
# Exercises each attack type for two rounds.
...
LOG_FILE="${LOG_DIR}/smoke_verify_attack_application_${TIMESTAMP}.log"

mkdir -p ${LOG_DIR}

echo "========================================" | tee -a ${LOG_FILE}
echo "Attack application verification" | tee -a ${LOG_FILE}
echo "========================================" | tee -a ${LOG_FILE}
...
for ATTACK in "${ATTACKS[@]}"; do
    ...
    if [ "${ATTACK_COUNT}" -gt 0 ] 2>/dev/null; then
        echo "[PASS] ${ATTACK}: PASS (${ATTACK_COUNT} attacks applied)" | tee -a ${LOG_FILE}
    else
        echo "[FAIL] ${ATTACK}: FAIL (no attacks applied)" | tee -a ${LOG_FILE}
    fi
...
for ATTACK in "${ATTACKS[@]}"; do
    ATTACK_COUNT=$(grep "Applied.*attack" ${LOG_FILE} | grep -c "${ATTACK}" || echo "0")
    if [ "${ATTACK_COUNT}" -gt 0 ] 2>/dev/null; then
        echo "  [PASS] ${ATTACK}: ${ATTACK_COUNT} attacks applied" | tee -a ${LOG_FILE}
    else
        echo "  [FAIL] ${ATTACK}: NO attacks applied" | tee -a ${LOG_FILE}
    fi
done
