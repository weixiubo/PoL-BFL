...
运行位置：PoL-BFL/Code
用法：python3 experiments/scripts/analyze_rq1_mnist_clearance.py
"""
...
BASE_DIR_GPU0 = Path("experiments/results/clearance/rq1_mnist_smoke_gpu0")
BASE_DIR_GPU1 = Path("experiments/results/clearance/rq1_mnist_smoke_gpu1")
...
def summarize_all():
    print("=" * 100)
    print("RQ1 MNIST 轻量清障结果总览（最终一轮指标）")
    print("=" * 100)
    print()
...
def check_expectations(all_data):
    ...
    if attack in all_data and "PoL_FL" in all_data[attack]:
        ...
        if tpr > 0.3:
            msg = f"  - PoL_FL: TPR={tpr:.3f} 显著>0，可能不符合我们对其局限性的预期"
            print(msg)
...
def main():
    if not BASE_DIR_GPU0.exists() or not BASE_DIR_GPU1.exists():
        print("结果目录不存在，请先完成 RQ1 MNIST 轻量实验。")
        return
