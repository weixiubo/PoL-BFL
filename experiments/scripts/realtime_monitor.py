...
def monitor_experiments():
    ...
    log_dir = Path('experiments/logs/parameter_evaluation')
    ...
    experiments = {
        'RQ1': 'rq1_cifar10_smoke.log',
        'RQ1 MNIST': 'rq1_mnist_tuning.log',
        'RQ2 MNIST': 'rq2_mnist_tuning.log',
    }

    print("\n" + "="*80)
    print("[CHECK] PoL-BFL EXPERIMENT MONITORING DASHBOARD")
    print("="*80)
    ...
    while True:
        ...
        print("\n" + "="*80)
        print(f"[RESULT] REAL-TIME MONITORING - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*80)
        ...
        for exp_name, log_file in experiments.items():
            ...
            if not log_path.exists():
                print(f"\n[WAITING] {exp_name:20} | Status: WAITING")
                all_completed = False
                continue
        ...
        print("[WAITING] Monitoring... (Press Ctrl+C to exit)")
        print("="*80)
