...
EXPERIMENTS = [
    ...
    {'phase': 1, 'rq': 'rq1', 'dataset': 'CIFAR10', 'rounds': 3, 'gpu': 0, 'tag': 'rq1_cifar10_quick'},
...
def run_experiment(exp_config: Dict) -> Tuple[str, bool, str]:
    ...
    try:
        ...
        if success:
            ...
            logger.error(f"❌ {tag} failed with code {result.returncode}")
        
        return tag, success, str(log_file)
    
    except subprocess.TimeoutExpired:
        logger.error(f"❌ {tag} timed out")
        return tag, False, str(log_file)
...
def main():
    """Main function"""
    logger.info("="*70)
    logger.info("🚀 PoL-BFL Complete Tuning Experiments")
    ...
    for phase in sorted(phases.keys()):
        logger.info(f"\n📋 Phase {phase}: {len(phases[phase])} experiments")
