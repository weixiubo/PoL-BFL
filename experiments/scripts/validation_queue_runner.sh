...
# Dual-GPU validation queue runner
...
# 主日志
MAIN_LOG="$LOG_DIR/validation_runner.log"
...
log "=========================================="
log "Dual-GPU validation queue started"
log "=========================================="
...
run_gpu0_queue() {
    ...
    for attack in "${GPU0_QUEUE[@]}"; do
        ...
        if [ $exit_code -eq 0 ]; then
            log "[GPU$gpu] [PASS] 完成: $attack"
        else
            log "[GPU$gpu] [FAIL] 失败: $attack (exit code: $exit_code)"
        fi
    done
...
log ""
log "=========================================="
log "Dual-GPU validation queue completed"
log "GPU0 退出码: $EXIT0"
log "GPU1 退出码: $EXIT1"
log "=========================================="
...
for attack in "${ATTACKS[@]}"; do
    ...
    if grep -q "Experiment completed successfully" "$exp_log" 2>/dev/null; then
        ...
        log "  [FAIL] $attack (检查日志: $exp_log)"
    else
        log "  [WARNING]  $attack (日志不存在)"
    fi
done
