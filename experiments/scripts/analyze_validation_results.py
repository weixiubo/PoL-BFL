...
def main():
    print("=" * 120)
    print(f"RQ1 MNIST 验证实验数据质量分析 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 120)
    print()
    ...
    for method in methods:
        ...
        if method_results:
            ...
            if valid_results:
                ...
                if errors:
                    print(f"    [FAIL] 错误: {len(errors)} 个")
                    for e in errors[:3]:
                        print(f"       - {e['file']}: {e['error']}")
                if warnings:
                    print(f"    [WARNING]  警告: {len(warnings)} 个")
                    for w in warnings[:3]:
    ...
    if total_errors == 0 and total_warnings == 0:
        print()
        print("  [PASS] 所有数据质量检查通过。")
    else:
        print()
        print("  [WARNING]  发现异常，需要进一步检查")
