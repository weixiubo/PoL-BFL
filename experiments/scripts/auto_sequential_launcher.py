...
class ExperimentLauncher:
    def __init__(self):
        ...
        self.base_dir = self.project_root / 'experiments'
        self.log_dir = self.base_dir / 'logs' / 'parameter_evaluation'
        self.result_dir = self.base_dir / 'results' / 'parameter_evaluation'
    ...
    def launch_experiment(self, gpu_id, exp_config):
        ...
        env['PYTHONPATH'] = os.pathsep.join((
            ...
            str(self.project_root / 'experiments' / 'scripts' / 'utils'),
        ))
        env['POL_DATA_DIR'] = str(self.project_root / 'data')

        print(f"\n{'='*80}")
        print(f"[START] Launching: {exp_config['name']} on GPU {gpu_id}")
    ...
    def monitor_and_launch(self):
        ...
        print("\n" + "="*80)
        print("[CHECK] AUTO-SEQUENTIAL EXPERIMENT LAUNCHER")
        print("="*80)
