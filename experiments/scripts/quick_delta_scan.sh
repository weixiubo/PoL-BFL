...
# Quick Delta Parameter Scan
# Tests different delta values to find optimal balance between TPR and FPR
...
LOG_FILE="${LOG_DIR}/quick_delta_scan_${TIMESTAMP}.log"

mkdir -p ${LOG_DIR}

echo "========================================" | tee -a ${LOG_FILE}
echo "Quick Delta Parameter Scan" | tee -a ${LOG_FILE}
echo "========================================" | tee -a ${LOG_FILE}
    ...
    if tpr_match and fpr_match:
        ...
        if fpr < 0.1:
            if tpr > best_tpr or (tpr == best_tpr and fpr < best_fpr):
                ...
                best_tpr = tpr
                best_fpr = fpr
        elif best_delta is None:  # No good delta found yet
            if tpr > best_tpr or (tpr == best_tpr and fpr < best_fpr):
                best_delta = delta
                best_tpr = tpr
                best_fpr = fpr
...
if best_delta:
    ...
    if best_fpr < 0.1:
        print(f"  ✅ Excellent choice! FPR < 0.1")
    elif best_fpr < 0.2:
        print(f"  ✅ Good choice! FPR < 0.2")
    else:
        print(f"  ⚠️ Consider testing higher delta values to reduce FPR")
else:
    print("  ❌ No suitable delta found. Consider testing different values.")
