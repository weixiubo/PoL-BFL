...
def check_environment():
    ...
    print(f"[PASS] Python: {py_version}")
    ...
    if cuda_available:
        ...
        print(f"[PASS] CUDA: Available ({gpu_count} GPUs)")
    ...
    try:
        ...
        for dataset_name in ['MNIST', 'CIFAR10', 'CIFAR100']:
            try:
                ...
                print(f"[PASS] {dataset_name}: train={len(train_ds)}, test={len(test_ds)}")
            except Exception as e:
                checks[dataset_name] = {'status': 'FAILED', 'error': str(e)}
                print(f"[FAIL] {dataset_name}: {e}")
    except Exception as e:
        print(f"[FAIL] Dataset factory error: {e}")
        checks['error'] = str(e)
    ...
    try:
        ...
        for model_name, num_classes in models:
            try:
                ...
                print(f"[PASS] {model_name}: {param_count:,} parameters")
            except Exception as e:
                checks[model_name] = {'status': 'FAILED', 'error': str(e)}
                print(f"[FAIL] {model_name}: {e}")
    except Exception as e:
        print(f"[FAIL] Model factory error: {e}")
        checks['error'] = str(e)
...
def check_pol_components():
    """检查PoL组件"""
    print("\n" + "="*70)
    print("[SECURITY] PoL COMPONENTS CHECK")
    print("="*70)
...
def check_experiment_scripts():
    """检查实验脚本"""
    print("\n" + "="*70)
    print("[NOTE] EXPERIMENT SCRIPTS CHECK")
    print("="*70)
...
def check_config():
    ...
    try:
        ...
        print(f"[PASS] POL_CONFIG: {json.dumps(pol_check, indent=2)}")
        ...
        print(f"[PASS] EXPERIMENT_CONFIG: {json.dumps(exp_check, indent=2)}")
    except Exception as e:
...
def main():
    """主函数"""
    print("\n" + "="*70)
    print("[START] PoL-BFL ENVIRONMENT & FRAMEWORK CHECK")
    ...
    print(f"[PASS] Passed: {passed_checks}/{total_checks}")

    if passed_checks == total_checks:
        print("\n[PASS] All checks passed. Framework is ready for experiments.")
        return 0
    else:
        print(f"\n[WARNING] {total_checks - passed_checks} checks failed; review the preceding output.")
        return 1
