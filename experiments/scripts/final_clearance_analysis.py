...
def main():
        ...
        for csv_file in sorted(gpu_dir.glob('*.csv')):
            try:
                ...
                if attack is None or method is None:
                    print(f"⚠️  无法解析文件名: {csv_file.name}")
                    continue
    ...
    if len(nan_acc) > 0:
        print(f"❌ {len(nan_acc)} 个实验准确率为NaN")
    ...
    if len(wrong_rounds) > 0:
        print(f"❌ {len(wrong_rounds)} 个实验轮数不是5")
