...
def main():
    ...
    if results:
        ...
        if all_passed:
            logger.info("\n[PASS] All checks satisfy the configured thresholds.")
            logger.info(f"\nSelected verification_rate = {best_config['verification_rate']}")
        else:
            logger.info("\n[WARNING] 部分检查未通过，需要进一步优化")
            logger.info("\n建议:")
