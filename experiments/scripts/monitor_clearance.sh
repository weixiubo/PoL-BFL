...
OUTPUT_BASE="$PROJECT_ROOT/experiments/results/clearance"
...
# 检查状态文件
if [ -f "$OUTPUT_BASE/clearance_status.txt" ]; then
    echo -e "${CYAN}【启动信息】${NC}"
    cat "$OUTPUT_BASE/clearance_status.txt"
    echo ""
fi
    ...
    for pidfile in "$PID_DIR"/*.pid; do
        if [ -f "$pidfile" ]; then
            ...
            if ps -p $pid > /dev/null 2>&1; then
                ...
                echo -e "${RED}❌ $name (PID: $pid) - 已停止${NC}"
            fi
        fi
    done
else
    echo -e "${YELLOW}⚠️  未找到PID目录${NC}"
fi
echo ""
...
check_results() {
    ...
    if [ -d "$dir" ]; then
        ...
        if [ $csv_count -gt 0 ]; then
            echo -e "${GREEN}✅ $name: $csv_count CSV文件, $json_count JSON文件${NC}"
            ...
            echo -e "${YELLOW}⏳ $name: 等待结果...${NC}"
        fi
    else
        echo -e "${YELLOW}⏳ $name: 目录不存在${NC}"
    fi
    echo ""
}
...
echo -e "==========================================${NC}"
echo ""
echo "提示: 使用 'watch -n 60 bash experiments/scripts/monitor_clearance.sh' 自动刷新"
