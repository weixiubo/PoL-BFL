...
def generate_summary_report(all_results):
    ...
    if best_vr is not None:
        print(f"\n[PASS] 推荐 verification_rate: {best_vr}")
        print(f"   理由: FPR=0，准确率最高 ({best_vr_accuracy:.4f})")
    else:
        print("\n[WARNING] 未找到 FPR=0 的配置")
...
def main():
    ...
    # RQ1 默认配置
    default_log = log_dir / 'rq1_20251114_212528_smoke_validation_v2.log'
    ...
    for log_file in log_files:
        ...
        try:
            ...
            if results['experiments']:
                ...
                print(f"  [PASS] 成功解析 {len(results['experiments'])} 个实验")
            else:
                print(f"  [WARNING] 未找到实验结果（可能还在运行）")
