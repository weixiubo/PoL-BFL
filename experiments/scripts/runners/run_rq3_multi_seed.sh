#!/bin/bash
# Run RQ3 Scalability experiment with multiple seeds and save per-seed result files.
...
OUTDIR="${RESULTS_DIR}/rq3_scalability"
LOGDIR="${LOG_ROOT}/rq3_multi_seed_$(date +%Y%m%d_%H%M%S)"
...
for S in "${SEEDS[@]}"; do
  echo "==== Running RQ3 with SEED=${S} on GPU=${GPU} ===="
  export SEED=${S}
  nohup "${PYBIN}" run_rq3_scalability.py > "${LOGDIR}/rq3_seed_${S}.log" 2>&1
  # Save per-seed copy
  if [[ -f "${OUTDIR}/rq3_results.json" ]]; then
    cp -f "${OUTDIR}/rq3_results.json" "${OUTDIR}/rq3_results_seed${S}.json"
    echo "Saved: ${OUTDIR}/rq3_results_seed${S}.json"
  else
    echo "[ERROR] Expected result file not found: ${OUTDIR}/rq3_results.json"
  fi
  sleep 2
done
