#!/bin/bash
# RQ3 清障实验：MNIST + CIFAR10 各8轮对照
# 验证性能约束：PoL-FL ≤ 1.5× Vanilla, PoL-FL+ZKP ≤ 2× Vanilla

set -e

PYTHON_BIN="/home/wxb/miniconda3/envs/wxb__veryfl_pol/bin/python"
...
CUDA_VISIBLE_DEVICES=0 CUBLAS_WORKSPACE_CONFIG=:4096:8 PYTHONUNBUFFERED=1 \
  ...
  2>&1 | tee ${LOG_DIR}/rq3_overhead_mnist_8r_clearance.log
...
CUDA_VISIBLE_DEVICES=1 CUBLAS_WORKSPACE_CONFIG=:4096:8 PYTHONUNBUFFERED=1 \
  ...
  2>&1 | tee ${LOG_DIR}/rq3_overhead_cifar10_8r_clearance.log
...
echo ""
echo "日志位置:"
echo "  - ${LOG_DIR}/rq3_overhead_mnist_8r_clearance.log"
echo "  - ${LOG_DIR}/rq3_overhead_cifar10_8r_clearance.log"
