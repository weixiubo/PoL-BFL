...
def test_all_attacks():
    ...
    for attack_name, params in free_riding_attacks.items():
        try:
            ...
            if attack_name in ['no_training', 'minimal_update']:
                ...
                # Just check if should_train() works
    ...
    try:
        ...
        fake_identities = attack.create_identities()
        shared_update = attack.get_shared_model_update(model_state)
        logger.info(f"  ✅ sybil: PASSED (generated {len(fake_identities)} identities)")
    except Exception as e:
        logger.error(f"  ❌ sybil: FAILED - {e}")
        return False
...
def test_all_aggregators():
    ...
    models = [create_dummy_model() for _ in range(10)]
    ...
    for agg_name, params in aggregators.items():
        try:
            ...
            logger.info(f"  ✅ {agg_name}: PASSED")
        except Exception as e:
            logger.error(f"  ❌ {agg_name}: FAILED - {e}")
            import traceback
            traceback.print_exc()
            return False
    
    logger.info("\n✅ All aggregators passed!")
    return True
...
def test_attack_aggregator_combinations():
    ...
    benign_models = [create_dummy_model() for _ in range(8)]
    global_model = create_dummy_model()
    ...
    for attack_name, attack_params, agg_name, agg_params in test_cases:
        try:
            ...
            logger.info(f"  ✅ {attack_name} + {agg_name}: PASSED")
        except Exception as e:
            logger.error(f"  ❌ {attack_name} + {agg_name}: FAILED - {e}")
            import traceback
            traceback.print_exc()
            return False
...
def test_edge_cases():
    ...
    try:
        ...
        logger.info("  ⚠️  Empty list handled (returned empty dict)")
    except Exception as e:
        logger.info(f"  ✅ Empty list raises exception (expected): {type(e).__name__}")
    ...
    try:
        ...
        logger.info("  ✅ Single model aggregation: PASSED")
    except Exception as e:
        logger.error(f"  ❌ Single model aggregation: FAILED - {e}")
        return False
    ...
    try:
        ...
        logger.info("  ✅ All malicious models: PASSED")
    except Exception as e:
        logger.error(f"  ❌ All malicious models: FAILED - {e}")
        return False
...
def main():
    ...
    if all_passed:
        ...
        logger.error("❌ Some tests failed - please fix before running experiments")
        return 1
