#!/bin/bash
# Phase 1 GPU1 Queue: MNIST RQ2+RQ3+RQ4
# Created: 2025-11-11 21:24
...
# Main log file
MAIN_LOG="$LOG_DIR/phase1_gpu1_queue.log"

echo "========================================" | tee -a "$MAIN_LOG"
echo "Phase 1 GPU1 Queue Started" | tee -a "$MAIN_LOG"
...
run_experiment() {
    ...
    if $PYTHON "$SCRIPT_DIR/$script_name" $args > "$log_file" 2>&1; then
        echo "✅ Completed: $exp_name" | tee -a "$MAIN_LOG"
        echo "   Log: $log_file" | tee -a "$MAIN_LOG"
    else
        echo "❌ Failed: $exp_name" | tee -a "$MAIN_LOG"
        echo "   Log: $log_file" | tee -a "$MAIN_LOG"
    fi
...
if [ -f "$SCRIPT_DIR/run_rq2_ablation.py" ]; then
    ...
    echo "⚠️  RQ2 script not found, skipping RQ2 experiments" | tee -a "$MAIN_LOG"
    echo "" | tee -a "$MAIN_LOG"
fi
...
if [ -f "$SCRIPT_DIR/run_rq3_overhead.py" ]; then
    ...
    echo "⚠️  RQ3 script not found, skipping RQ3 experiments" | tee -a "$MAIN_LOG"
    echo "" | tee -a "$MAIN_LOG"
fi
...
if [ -f "$SCRIPT_DIR/run_rq4_incentive.py" ]; then
    ...
    echo "⚠️  RQ4 script not found, skipping RQ4 experiments" | tee -a "$MAIN_LOG"
    echo "" | tee -a "$MAIN_LOG"
fi
...
# ========================================
echo "========================================" | tee -a "$MAIN_LOG"
echo "Phase 1 GPU1 Queue Completed" | tee -a "$MAIN_LOG"
echo "Time: $(date)" | tee -a "$MAIN_LOG"
echo "========================================" | tee -a "$MAIN_LOG"
echo "" | tee -a "$MAIN_LOG"
...
echo "⚠️  Skipped: $(grep -c "⚠️  " "$MAIN_LOG" || echo 0)" | tee -a "$MAIN_LOG"
echo "" | tee -a "$MAIN_LOG"
