...
- Repetitions: 1
- Variant: pol_only (PoL-only profile)
...
def main():
    parser = argparse.ArgumentParser(description='Test checkpoint verification')
    ...
    logger.info("=" * 70)
    logger.info("Checkpoint Verification Test")
    logger.info("=" * 70)
    ...
    try:
        ...
        # 评估修正效果
        logger.info("\n" + "=" * 70)
        logger.info("Verification Evaluation")
        logger.info("=" * 70)

        if results['tpr'] >= 0.90:
            logger.info("[PASS] TPR >= 90%: PASS")
        else:
            logger.warning(f"[WARNING] TPR = {results['tpr']:.1%} < 90%: NEEDS IMPROVEMENT")

        if results['fpr'] <= 0.10:
            logger.info("[PASS] FPR <= 10%: PASS")
        else:
            logger.warning(f"[WARNING] FPR = {results['fpr']:.1%} > 10%: NEEDS IMPROVEMENT")
        ...
        logger.info("=" * 70)
        logger.info("The verification log records:")
