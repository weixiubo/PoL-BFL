...
Quick test to verify all experiment components work correctly.
"""
...
def test_data_loading():
    ...
    try:
        ...
        logger.info(f"✓ Loaded MNIST: {len(dataset)} samples")
        
        # Partition data
        client_datasets = partition_data_iid(dataset, num_clients=5)
        logger.info(f"✓ Partitioned into {len(client_datasets)} clients")
        
        # Get statistics
        stats = get_data_statistics(client_datasets)
        logger.info(f"✓ Statistics: {stats['num_clients']} clients, {stats['total_samples']} samples")
...
def test_model_creation():
    ...
    try:
        ...
        logger.info(f"✓ Created SimpleCNN: {num_params:,} parameters")
        
        # Test forward pass
        x = torch.randn(2, 1, 28, 28)
        y = model(x)
        logger.info(f"✓ Forward pass: input {x.shape} -> output {y.shape}")
...
def test_aggregators():
    ...
    try:
        ...
        logger.info(f"✓ FedAvg aggregation: {len(aggregated)} parameters")
        
        # Test Krum
        aggregator = create_aggregator('Krum', num_byzantine=0)
        aggregated = aggregator.aggregate(models)
        logger.info(f"✓ Krum aggregation: {len(aggregated)} parameters")
...
def test_metrics():
    ...
    try:
        ...
        logger.info(f"✓ MetricsTracker: latest accuracy = {latest}")
        ...
        time.sleep(0.1)
        elapsed = timer.stop()
        logger.info(f"✓ Timer: elapsed = {elapsed:.3f}s")
        ...
        # Take only first batch for quick test
        ...
        logger.info(f"✓ Accuracy computation: {acc:.4f} (random model)")
