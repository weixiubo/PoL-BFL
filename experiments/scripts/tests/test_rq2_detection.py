"""
Reduced-scale test of RQ2 detection
...
if __name__ == '__main__':
    # Smoke test config: 1 round, 1 repetition
    test_config = {
        'dataset': 'MNIST',
        'model': 'SimpleCNN',
        'num_clients': 20,
    ...
    logger.info("="*70)
    logger.info("Testing RQ2 detection")
    logger.info("="*70)
    ...
    if result['detection_metrics']['TPR'] > 0 or result['detection_metrics']['FPR'] > 0:
        logger.info("[PASS] SUCCESS: Detection is working.")
    else:
        logger.warning("[WARNING] WARNING: Detection metrics still 0, may need further investigation")

    if result['participation_rate'] > 0:
        logger.info("[PASS] SUCCESS: Participation rate is working.")
    else:
        logger.warning("[WARNING] WARNING: Participation rate still 0, may need further investigation")
