#!/bin/bash

# Simple experiment monitoring script

LOG_DIR="experiments/logs/parameter_evaluation"
RESULT_DIR="experiments/results/parameter_evaluation"

echo "=========================================="
echo "PoL-BFL Experiment Monitor"
echo "=========================================="

while true; do
    clear
    echo "=========================================="
    echo "PoL-BFL Experiment Monitor - $(date '+%Y-%m-%d %H:%M:%S')"
    echo "=========================================="

    echo ""
    echo "[RESULT] GPU Status:"
    nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader

    echo ""
    echo "[PLAN] Running Processes:"
    ps aux | grep "run_rq" | grep -v grep | wc -l
    ps aux | grep "run_rq" | grep -v grep | awk '{print "  - " $NF}'

    echo ""
    echo "[PATH] Log Files:"
    for log in $LOG_DIR/*.log; do
        if [ -f "$log" ]; then
            size=$(du -h "$log" | cut -f1)
            lines=$(wc -l < "$log")
            name=$(basename "$log")
            echo "  $name: $size ($lines lines)"

            # Show last accuracy
            if grep -q "Test Accuracy:" "$log"; then
                acc=$(grep "Test Accuracy:" "$log" | tail -1 | awk '{print $NF}')
                echo "    └─ Latest Accuracy: $acc"
            fi
        fi
    done

    echo ""
    echo "[RESULT] Result Directories:"
    for dir in $RESULT_DIR/*/; do
        if [ -d "$dir" ]; then
            name=$(basename "$dir")
            files=$(find "$dir" -type f | wc -l)
            size=$(du -sh "$dir" | cut -f1)
            echo "  $name: $files files, $size"
        fi
    done

    echo ""
    echo "[WAITING] Waiting 30 seconds... (Press Ctrl+C to exit)"
    sleep 30
done

