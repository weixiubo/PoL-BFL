#!/usr/bin/env python3
"""Plan final-paper Table 4 PoL-BFL prefilter compositions."""
...
class ComposabilityCell:
    ...
    @property
    def run_id(self) -> str:
        return f"formal-composability-{self.aggregation}-{self.attack.lower()}-s{self.seed}"


def plan_composability_cells(
    matrix: Mapping[str, object],
    *,
    aggregations: Sequence[str] | None = None,
    attacks: Sequence[str] | None = None,
    seeds: Sequence[int] | None = None,
) -> tuple[ComposabilityCell, ...]:
    study = matrix["studies"]["table_4_composability"]
    aggregation_map = {"Krum": "krum", "TrimmedMean": "trimmed_mean", "Median": "median"}
    selected_aggregations = tuple(aggregations or aggregation_map.values())
    selected_attacks = tuple(attacks or study["attacks"])
    selected_seeds = tuple(int(seed) for seed in (seeds or matrix["seeds"]))
    if (
        set(selected_aggregations) - set(aggregation_map.values())
        or set(selected_attacks) - set(study["attacks"])
        or set(selected_seeds) - {int(seed) for seed in matrix["seeds"]}
    ):
        raise ValueError("composability matrix filter contains unknown values")
    return tuple(
        ComposabilityCell(aggregation, attack, seed)
...
def composability_command(
    cell: ComposabilityCell,
    *,
    python: Path,
    output: Path,
    data_root: Path,
    ...
    command = [
        ...
        "--dataset", "CIFAR10",
        "--attack", cell.attack,
        "--aggregation-method", cell.aggregation,
        "--seed", str(cell.seed),
        "--run-id", cell.run_id,
        "--output", str(output),
...
def main() -> None:
    ...
    parser.add_argument("--results-root", type=Path, default=ROOT / "experiments" / "results" / "final" / "composability")
    ...
    cells = plan_composability_cells(
        matrix,
    ...
    if source["dirty"] or not source["commit"]:
        raise RuntimeError("formal composability execution requires a clean, identified source")
    environment = os.environ.copy()
    environment.update({"POL_INTEGRITY": "1", "CUBLAS_WORKSPACE_CONFIG": ":4096:8", "CUDA_VISIBLE_DEVICES": "0,1", "OMP_NUM_THREADS": "2"})
    for cell in cells:
        ...
        if result_path.is_file():
            if json.loads(result_path.read_text(encoding="utf-8")).get("formal_accepted") is True:
                continue
            raise RuntimeError(f"refusing to overwrite rejected composition: {result_path}")
