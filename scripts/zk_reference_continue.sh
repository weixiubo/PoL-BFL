...
PTAU="${POL_ZK_PTAU:?POL_ZK_PTAU must reference the verified phase-2 transcript}"
...
"$NODE_BIN" "$SNARKJS_CLI" zkey verify "$R1CS" "$PTAU" "$ZKEY_FINAL" | tee "$BUILD_DIR/zkey-verify.log"
"$NODE_BIN" "$SNARKJS_CLI" zkey export verificationkey "$ZKEY_FINAL" "$VKEY"
"$NODE_BIN" "$SNARKJS_CLI" zkey export solidityverifier "$ZKEY_FINAL" "$BUILD_DIR/Groth16Verifier.sol"
