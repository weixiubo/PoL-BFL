...
def test_zkp_pol_verifier():
    ...
    try:
        ...
        if not is_valid:
            logger.error(f"❌ Verification failed: {detail}")
            return False
        
        logger.info("✅ Single step verification passed")
        ...
        if not is_valid:
            logger.error(f"❌ Checkpoint verification failed: {detail}")
            return False
        ...
        if not is_valid:
            logger.error(f"❌ Training trajectory verification failed")
            return False
        
        logger.info("✅ Training trajectory verification passed")
        ...
        logger.error(f"❌ ZKP-PoL Verifier test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_zkp_pol_aggregator():
    ...
    try:
        ...
        if not is_valid:
            logger.error(f"❌ Aggregation failed")
            return False
        
        logger.info("✅ Aggregation with ZKP verification passed")
        ...
        logger.error(f"❌ ZKP-PoL Aggregator test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_verification_stats():
    ...
    try:
        ...
        logger.error(f"❌ Verification stats test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    """运行所有测试"""
    logger.info("\n" + "="*60)
    logger.info("🧪 ZKP-PoL Integration Tests")
    logger.info("="*60)
    ...
    if passed == total:
        logger.info("\n🎉 All ZKP-PoL integration tests passed!")
        return 0
    else:
