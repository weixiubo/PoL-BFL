#!/bin/bash
# 缩减规模检查CIFAR-10实验状态（已迁移到 scripts/ 目录）
...
if ps aux | grep run_cifar10_paper | grep -v grep > /dev/null; then
    echo "   [PASS] Experiment is RUNNING"
    ps aux | grep run_cifar10_paper | grep -v grep | awk '{print "   PID: " $2 ", CPU: " $3 "% , MEM: " $4 "%"}'
else
    echo "   [WARNING]  Experiment is NOT running (may have completed or crashed)"
fi
echo ""
...
if [ -f cifar10_experiment.log ]; then
    ...
    echo "   [WARNING]  No accuracy data yet"
fi
echo ""
...
if [ -f experiments/results/cifar10_paper/results.json ]; then
    ...
    echo "   [WARNING]  Results not generated yet"
fi
echo ""
