...
def test_zkp_pol_integration():
    ...
    try:
        ...
        logger.info("✅ ZKPPoLVerifier initialized successfully")
        
        # 测试验证流程
        logger.info("✅ ZKP-PoL verification system ready")
...
def test_economic_incentive_integration():
    ...
    try:
        ...
        logger.info("✅ EconomicIncentiveSystem initialized successfully")
        
        # 注册客户端
        for i in range(3):
            success, msg = system.register_client(f'client_{i}', 150.0)
            if not success:
                logger.error(f"❌ Failed to register client: {msg}")
                return False
        
        logger.info("✅ Clients registered successfully")
        
        # 验证资格
        for i in range(3):
            eligible, reason = system.verify_client_eligibility(f'client_{i}')
            if not eligible:
                logger.error(f"❌ Client eligibility check failed: {reason}")
                return False
        
        logger.info("✅ All clients are eligible")
        ...
        for i in range(3):
            ...
            if 'error' in result:
                logger.error(f"❌ Verification processing failed: {result['error']}")
                return False
        
        logger.info("✅ Verification results processed successfully")
        
        # 结束轮次
        round_stats = system.end_round()
        logger.info(f"✅ Round ended: {round_stats}")
    ...
    try:
        ...
        for round_num in range(2):
            ...
            if round_num == 0:
                ...
                logger.info("✅ Clients registered")
...
def main():
    """运行所有测试"""
    logger.info("\n" + "="*60)
    logger.info("🧪 Phase 3 & 4 Integration Tests")
    logger.info("="*60)
    ...
    if passed == total:
        logger.info("\n🎉 All Phase 3 & 4 integration tests passed!")
        return 0
    else:
