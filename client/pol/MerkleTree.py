...
def test_merkle_tree():
    ...
    fake_leaf = hashlib.sha256(b"fake_data").hexdigest()
    proof = tree.get_proof(0)
    is_valid = MerkleTree.verify_proof(fake_leaf, proof, tree.get_root())
    print(f"  Fake leaf: valid={is_valid}")
    assert not is_valid, "Proof for fake leaf should be invalid"
